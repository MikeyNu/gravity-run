...
async function sizeOf(path: string): Promise<number | null> {
  try { const metadata = await stat(resolve(repositoryRoot, path)); return metadata.isFile() ? metadata.size : null; }
  catch { return null; }
}
...
for (const asset of manifest.assets ?? []) {
  ...
  ids.add(asset.id);
  if (!asset.source.startsWith('content/source/') && !asset.source.startsWith('docs/reference/') && !asset.source.startsWith('apps/game/public/')) errors.push(`${asset.id}: source must be under content/source/, docs/reference/, or apps/game/public/.`);
  ...
  if (bytes !== null && bytes > asset.budget.transferKilobytes * 1024) errors.push(`${asset.id}: ${Math.ceil(bytes / 1024)} KB exceeds ${asset.budget.transferKilobytes} KB budget.`);
